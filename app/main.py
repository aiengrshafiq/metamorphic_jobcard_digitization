...
def _load_config() -> dict:
    ...
    try:
        ...
        log.warning("Config file %s not found; continuing with empty config.", path)
        return {}
    except Exception:
        log.exception("Failed to read %s; continuing with empty config.", path)
        return {}
...
# ... (The rest of the file uses app_config.get() for safety)
# --- The rest of the file with all endpoints and logic ---
...
async def read_duty_officer_form(request: Request, db: Session = Depends(get_db)):
    context = {"request": request, "page_title": "Metamorphic • Duty Officer Progress", "job_cards": db.query(JobCard).options(joinedload(JobCard.project)).order_by(JobCard.id.desc()).all(), "foremen": db.query(Foreman).order_by(Foreman.name).all(), "equipment_conditions": app_config.get('equipment_conditions', []), "sub_contractor_coordinations": app_config.get('sub_contractor_coordinations', []), "delivery_statuses": app_config.get('delivery_statuses', []), "sop_statuses": app_config.get('sop_statuses', []),}
...
async def read_site_officer_form(request: Request, db: Session = Depends(get_db)):
    context = {"request": request, "page_title": "Metamorphic • Sites Officers Daily Progress Report", "site_locations": app_config.get('site_locations', []), "supervisors": db.query(Supervisor).order_by(Supervisor.name).all(), "foremen": db.query(Foreman).order_by(Foreman.name).all(), "job_cards": db.query(JobCard).options(joinedload(JobCard.project)).order_by(JobCard.id.desc()).all(), "projects": db.query(Project).order_by(Project.name).all(), "subcontractor_coordination_c": app_config.get('subcontractor_coordination_c', []), "site_condition_options": app_config.get('site_condition_options', []), "overall_site_health_options": app_config.get('overall_site_health_options', []), "yes_no_help_options": app_config.get('yes_no_help_options', []),}
...
async def job_card_tracking(request: Request, db: Session = Depends(get_db)):
    job_cards = db.query(JobCard).options(joinedload(JobCard.project), selectinload(JobCard.tasks)).order_by(JobCard.id.desc()).all()
    context = {"request": request, "page_title": "Job Card Tracking", "job_cards": job_cards, "task_statuses": app_config.get('task_statuses', [])}
...
async def read_material_requisition_form(request: Request, db: Session = Depends(get_db)):
    context = {"request": request, "page_title": "Material Requisition Form", "projects": db.query(Project).order_by(Project.name).all(), "supervisors": db.query(Supervisor).order_by(Supervisor.name).all(), "material_types": app_config.get('material_types', []), "urgency_levels": app_config.get('urgency_levels', []),}
...
async def process_material_requisition_form(req_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    context = {"request": request, "page_title": f"Process Requisition #{req.id}", "req": req, "suppliers": db.query(Supplier).order_by(Supplier.name).all(), "approval_statuses": app_config.get('approval_statuses', []), "requisition_statuses": app_config.get('requisition_statuses', []),}
...
async def update_task_status(task_id: int, db: Session = Depends(get_db), status: str = Form(...)):
    ...
    pending_or_processing_tasks_count = db.query(Task).filter(Task.job_card_id == job_card.id, Task.status != 'Done').count()
