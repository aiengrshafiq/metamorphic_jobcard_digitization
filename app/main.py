...
from app.models import (
    ...
    DutyOfficerProgress, SiteOfficerReport, MaterialRequisition, Supplier
)
...
class JobCardAdmin(ModelView, model=JobCard):
    ...
    column_details_list = [
        JobCard.id, JobCard.job_card_no, JobCard.project, JobCard.date_issued,
        JobCard.site_location, JobCard.site_engineer, JobCard.supervisor,
        JobCard.foreman, JobCard.tasks, JobCard.progress_reports, JobCard.site_officer_reports
    ]
...
class DutyOfficerProgressAdmin(ModelView, model=DutyOfficerProgress):
    ...
    column_details_list = [c.name for c in DutyOfficerProgress.__table__.c] + [
        DutyOfficerProgress.job_card, DutyOfficerProgress.foreman_signature
    ]
    name_plural = "Progress Reports (Form B)"

class SiteOfficerReportAdmin(ModelView, model=SiteOfficerReport):
    column_list = [SiteOfficerReport.id, SiteOfficerReport.date, SiteOfficerReport.site_location, "job_card"]
    column_formatters = {"job_card": lambda m, a: m.job_card.job_card_no if m.job_card else ""}
    column_details_list = [c.name for c in SiteOfficerReport.__table__.c] + [
        SiteOfficerReport.site_officer, SiteOfficerReport.duty_officer, SiteOfficerReport.job_card,
        SiteOfficerReport.material_requisition_project
    ]
...
# --- Procurement Workflow Endpoints (NEW) ---
...
async def update_material_requisition(req_id: int, db: Session = Depends(get_db),
    supplier_id: int = Form(...), status: str = Form(...),
    lpo_number: Optional[str] = Form(None), pm_approval: Optional[str] = Form(None),
    qs_approval: Optional[str] = Form(None), payment_status: Optional[str] = Form(None),
    remarks: Optional[str] = Form(None)
):
    req = db.query(MaterialRequisition).filter(MaterialRequisition.id == req_id).first()
    if not req: raise HTTPException(status_code=404, detail="Requisition not found")
    try:
        # Generate MR Number only if it doesn't exist
